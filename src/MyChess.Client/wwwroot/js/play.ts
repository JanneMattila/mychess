...
MyChessPlay.pieceSelected = (evt) => {
    console.log(evt);
    //evt.preventDefault();
}

MyChessPlay.draw = (board: any) => {
    console.log(board);
    ...
    for (var row = 0; row < board.length; row++) {
        ...
        for (var column = 0; column < r.length; column++) {
            ...
            let cell = `<td id='${item.key}' width='${pieceSize}px' height='${pieceSize}px' class='${item.background}' onclick='MyChessPlay.pieceSelected'>`
